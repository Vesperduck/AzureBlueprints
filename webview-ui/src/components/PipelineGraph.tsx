...
export default function PipelineGraph({
  ...
  const handleConnect = useCallback(
    (connection: Connection) => {
      ...
      } else if (sourceNode?.data.kind === 'job') {
        // 1:1 job→task enforcement: a job may have at most one direct outgoing
        // edge to a task node. Remove any existing job→task edges from this
  ...
  const handleConnectEnd = useCallback(
    (event: MouseEvent | TouchEvent) => {
      ...
      // AND the source was a trigger, stage, job, or task node.
      if (connectCompleted.current) { return; }
      const sourceKind = connectSource.current?.kind;
      const isTaskKind = sourceKind === 'task' || sourceKind === 'script' ||
        sourceKind === 'checkout' || sourceKind === 'publish' || sourceKind === 'download';
      if (!connectSource.current || (sourceKind !== 'trigger' && sourceKind !== 'stage' && sourceKind !== 'job' && !isTaskKind)) { return; }
